<template>
	<div id="top-navigation-bar">
		<v-toolbar
				class="primary"
				app
				absolute
				clipped-left
		>
			<v-toolbar-side-icon class="white--text" to="/dashboard"></v-toolbar-side-icon>
			<v-toolbar-title>
				<span class="headline white--text">{{ title }}</span>
			</v-toolbar-title>
			<v-spacer></v-spacer>
			<v-toolbar-items>
				<v-btn flat class="white--text" to="/add-technician-sales">Add Technician Sales</v-btn>
				<v-btn flat class="white--text" to="/update-technician-sales">Update Technician Sales</v-btn>
				<v-btn flat class="white--text" to="/settings">Settings</v-btn>
			</v-toolbar-items>
			<v-menu offset-y>
				<v-btn slot="activator" class="white--text" flat>
					{{ userFullName }}
					<v-icon right>keyboard_arrow_down</v-icon>
				</v-btn>
				<v-list>
					<v-list-tile v-for="(item, key) in items" :key="key" @click="logout">
						<v-list-tile-title>{{ item.name }}</v-list-tile-title>
					</v-list-tile>
				</v-list>
			</v-menu>
		</v-toolbar>
	</div>
</template>

<script>
export default {
	name: "TopNavigationBar",
	props: {
		title: {
			type: String,
			required: true
		}
	},
	data () {

		return {
			items: [
				{ name: "Logout" }
			]
		};

	},
	computed: {

		userFullName () {

			return this.$store.getters["Authentications/userFullName"];

		}

	},
	methods: {
		logout () {

			this.$store.dispatch("Authentications/logout");

		}

	},
};

</script>

<style scoped>

</style>
